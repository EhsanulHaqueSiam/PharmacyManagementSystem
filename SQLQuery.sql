...
CREATE TABLE Admin (
    ...
    A_Dob DATE,
    A_Number VARCHAR(15) CHECK (LEN(A_Number) BETWEEN 10 AND 15), -- Added validation for phone number length
...
CREATE TABLE Pharmacist (
    ...
    P_Dob DATE,
    P_Number VARCHAR(15) CHECK (LEN(P_Number) BETWEEN 10 AND 15), -- Added validation for phone number length
...
CREATE TABLE Customer (
    ...
    C_Number VARCHAR(15) CHECK (LEN(C_Number) BETWEEN 10 AND 15), -- Added validation for phone number length
...
CREATE TABLE Transactions (
    ...
    C_ID INT FOREIGN KEY REFERENCES Customer(C_ID) ON DELETE CASCADE -- Ensuring referential integrity
);
...
CREATE TABLE CustomerCoupon (
    C_ID INT FOREIGN KEY REFERENCES Customer(C_ID) ON DELETE CASCADE,
    C_ID_Coupon INT FOREIGN KEY REFERENCES Coupon(C_ID) ON DELETE CASCADE, -- Renamed column for consistency
